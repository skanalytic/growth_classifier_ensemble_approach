...
## Load generic caret models

```{r Caret}


Caret_ML_models <- read_csv("Caret_ML_models.csv")

# TYPE SPLIT
Caret_ML_models$Classification = ifelse(grepl('Classification',Caret_ML_models$Type) == TRUE,1,0)
Caret_ML_models$Regression = ifelse(grepl('Regression',Caret_ML_models$Type) == TRUE,1,0)


# LIBRARY SPLIT
Caret_ML_models$Library[is.na(Caret_ML_models$Library)] = "caret"

mainliblist = c()
for (liblist in Caret_ML_models$Library){
  mainlib = unlist(strsplit(liblist,','))
  mainlib = mainlib[i]
  mainliblist = append(mainliblist,mainlib)
}
mainliblist = data.frame(mainliblist)
names(mainliblist) = ""
col = paste("Library",i,sep="_")
Caret_ML_models[col] = data.frame(mainliblist) 
}
...
for (i in c(1:8)){
mainparlist = c()
for (parlist in Caret_ML_models$Parameters){
  mainpar = unlist(strsplit(parlist,','))
  mainpar = mainpar[i]
  mainparlist = append(mainparlist,mainpar)
}
mainparlist = data.frame(mainparlist)
names(mainparlist) = ""
col = paste("Parameter",i,sep="_")
Caret_ML_models[col] = data.frame(mainparlist) 
}

# SAVE TO FILE
#write.csv(Caret_ML_models,'CARET_MODELS_SK.csv')

# INSTALL ALL PACKAGES
#for (lib in Caret_ML_models$Library_1){install.packages(lib)}
...
ttmanualclass_LTM <- ttmanualclass[,rep(1:ncol(ttmanualclass), 
                                        times = rep(12,ncol(ttmanualclass)))]
...
## -------------------------------------------------------- DATA ANALYSIS OF REAL DATA TO USE AS COMPARISON TO MODEL OUTPUTS -----------------------

## switch rate analysis - historic data 

```{r}

df_full_target = melt(ttmanualclass_ANNREP)
df_full = cbind(df_full_target,df_full_variables)
df_full = df_full[,3:ncol(df_full)]

df_full$value = as.factor(droplevels(df_full$value))
df_full$TIME = df_full$`rep(colnames(BSASSETS_ANN), each = nrow(BSASSETS_ANN))`
df_full$STOCK_ID = df_full$`rep(rownames(BSASSETS_ANN), ncol(BSASSETS_ANN))`
df_full$UNIQUE_ID = paste(df_full$STOCK_ID,df_full$TIME)

final2 = df_full %>% dplyr::select(value,STOCK_ID,TIME,UNIQUE_ID)
names(final2)
names(final2) = c("TARGET","STOCK_ID","TIME","UNIQUE_ID")
final2$TIME <- gsub('X', '', final2$TIME)
final2$TIME =  as.Date(final2$TIME, "%Y%m%d")

# OR LAG AND 1|0

SKaddtargetlags <- function(df = NULL, n = NULL){
  df$UNIQUE_ID = rownames(df)
  dfGlag = df %>% dplyr::select(STOCK_ID,TIME,TARGET,UNIQUE_ID) %>% arrange(STOCK_ID,TIME)
  
  dfGlag = dfGlag %>% group_by(STOCK_ID) %>% mutate(LAGN = dplyr::lag(TARGET, n = n, default = NA)) %>% ungroup()

  dfGlag = dfGlag %>% dplyr::select(UNIQUE_ID,LAGN)
  names(dfGlag)
  
  df = merge(df,dfGlag,by='UNIQUE_ID')
  df = df %>% dplyr::select(-UNIQUE_ID)
  df
}

df_lag = SKaddtargetlags(final2,n=1)

df_lag$LAGBINARY = ifelse(df_lag$TARGET == df_lag$LAGN,0,1)
df_lag$TIME_Y = format(df_lag$TIME,"%Y")

df_switch = df_lag %>% select(TIME_Y, LAGBINARY) %>% group_by(TIME_Y) %>% summarise(s = sum(LAGBINARY,na.rm=T), n = n()) %>% mutate(p = s/n)
df_switch
ggplot(data=df_switch, aes(x=TIME_Y, y= p)) + geom_point()

write.csv(df_switch,'REAL_switch_distribution.csv')

```

## Up/down sales growth analysis - historic data 

```{r}


df_full_target = melt(ttmanualclass_ANNREP)
df_full_variables = predvalsgrowthEVAlogit_ANN
df_full = cbind(df_full_target,df_full_variables)
df_full = df_full[,3:ncol(df_full)]

df_full$value = as.factor(droplevels(df_full$value))
names(df_full)[names(df_full) == 'value'] = 'TARGET'
df_full$TIME = df_full$`rep(colnames(BSASSETS_ANN), each = nrow(BSASSETS_ANN))`
df_full$STOCK_ID = df_full$`rep(rownames(BSASSETS_ANN), ncol(BSASSETS_ANN))`
df_full$UNIQUE_ID = paste(df_full$STOCK_ID,df_full$TIME)

df_full$TIME <- gsub('X', '', df_full$TIME)
df_full$TIME =  as.Date(df_full$TIME, "%Y%m%d")

df_full$sales


dfGlag = df_full %>% dplyr::select(STOCK_ID,TIME,as.vector.SalesGrowthfull.,UNIQUE_ID) %>% arrange(STOCK_ID,TIME)
  
dfGlag = dfGlag %>% group_by(STOCK_ID) %>% mutate(LAGN = dplyr::lag(as.vector.SalesGrowthfull., n = n, default = NA)) %>% ungroup()

dfGlag = dfGlag %>% dplyr::select(UNIQUE_ID,LAGN)

df_full2 = merge(df_full,dfGlag,by='UNIQUE_ID')
df_full2 = df_full2 %>% dplyr::select(-UNIQUE_ID)

df_full3 = df_full2 %>% select(STOCK_ID,TIME,TARGET,as.vector.SalesGrowthfull.,LAGN)

df_full3 = df_full3 %>% mutate(salesgrowthdif1 = as.vector.SalesGrowthfull. - LAGN)
df_full3$salesgrowthdif1[df_full3$salesgrowthdif1==0] <- NA
df_full3$salesgrowthdif1_binary = ifelse(df_full3$salesgrowthdif1 > 0,"UP","DOWN")

up = df_full3 %>% select(salesgrowthdif1_binary,TARGET) %>% group_by(TARGET,salesgrowthdif1_binary) %>% summarise(n=n()) %>% filter(salesgrowthdif1_binary == 'UP')
up  = up[,c(1,3)]
names(up) = c("TARGET","UP_n")
down = df_full3 %>% select(salesgrowthdif1_binary,TARGET) %>% group_by(TARGET,salesgrowthdif1_binary) %>% summarise(n=n()) %>% filter(salesgrowthdif1_binary == 'DOWN')
down  = down[,c(1,3)]
names(down) = c("TARGET","DOWN_n")
all = merge(up,down,by="TARGET")
all = all %>% mutate(total_obs = UP_n + DOWN_n)
all = all %>% mutate(ud_ratio = UP_n - DOWN_n / total_obs)
...
df_full4 = df_full3[complete.cases(df_full3), ]

up = df_full4 %>% select(STOCK_ID,salesgrowthdif1_binary,TARGET) %>% group_by(STOCK_ID,salesgrowthdif1_binary) %>% summarise(n=n()) %>% filter(salesgrowthdif1_binary == 'UP')
down = df_full4 %>% select(STOCK_ID,salesgrowthdif1_binary,TARGET) %>% group_by(STOCK_ID,salesgrowthdif1_binary) %>% summarise(n=n()) %>% filter(salesgrowthdif1_binary == 'DOWN')
all = merge(up,down,by="STOCK_ID")
all = all %>% mutate(total = n.x + n.y)
all = all %>% mutate(ratioud = (n.x - n.y) / total)


MC = df_full4 %>% select(STOCK_ID,TARGET) %>% group_by(STOCK_ID,TARGET) %>% summarise(n=n()) %>% filter(TARGET == 'MATURECYCLICAL')
MS = df_full4 %>% select(STOCK_ID,TARGET) %>% group_by(STOCK_ID,TARGET) %>% summarise(n=n()) %>% filter(TARGET == 'MATURESTABLE')
all2 = merge(MC,MS,by="STOCK_ID")
all2 = all2 %>% mutate(totalM = n.x + n.y)
all2 = all2 %>% mutate(ratioMC = (n.x - n.y) / totalM)

all3 = merge(all,all2,by="STOCK_ID")
all4 = all3 %>% select(STOCK_ID,ratioud,ratioMC)
...
# dont see a big connection between ratio of UPs/Downs - observations, and ratio of MC labels to MS 

```

## Distribution analysis - historic data 
...
ncol((ttmanualclass_ANNREP)) # * 219 rows 237 cols, 51903 when melted 
...
# rename columns 
df = df_full
names(df)[names(df) == 'value'] <- 'TARGET'
names(df)[names(df) == "rep(rownames(BSASSETS_ANN), ncol(BSASSETS_ANN))"] <- 'STOCK_ID'
names(df)[names(df) == "rep(colnames(BSASSETS_ANN), each = nrow(BSASSETS_ANN))"] <- 'TIME'

# convert time to date 
df$TIME <- gsub('X', '', df$TIME)
df$TIME =  as.Date(df$TIME, "%Y%m%d")

df = df %>% dplyr::select(-STOCK_ID)

#################################### DO DISTRIBUTION ANALYSIS FOR REAL DATA TO COMPARE ##################################################

#Q: how many growths are there per year as percentage of total number in that year (data needed: TIME, TARGET)

dfdist = df %>% dplyr::select(TIME,TARGET)
dfdist$TIME = format(dfdist$TIME,"%Y")

dfdist2 = dfdist
dfdist2$TARGET = ifelse(dfdist2$TARGET == "MATURECYCLICAL", "NOT_GROWTH", ifelse(dfdist2$TARGET == "GROWTH", "GROWTH", "NOT_GROWTH"))
dfdisttotal = dfdist2 %>% group_by(TIME) %>% summarise(total = n())

dfdistsplit = dfdist2 %>% group_by(TIME,TARGET) %>% summarise(n = n())

dfdist3 = merge(dfdisttotal,dfdistsplit,by='TIME')
dfdist3 = dfdist3 %>% mutate(perc = n/total)

dffist4 = dfdist3 %>% filter(TARGET == 'GROWTH')
dffist4
ggplot(data=dffist4, aes(x=TIME,y=perc)) + geom_point()


```


## ----------------------------------------------------- PREVIOUS BENCHMARK CHECKS ---------------------------------------------

# Previous benchmark model - function (plus parallel processing)
...
FZhibacktestnonode <- function(trainingmx=NULL,predictionmx=NULL, class_method1=NULL, class_method2=NULL, train_method=NULL, useweights=FALSE,binary=FALSE,maxmtry = 35, node=NULL){
  ...
  registerDoParallel(cl)
	...
	if(binary){
		...
		trctrl <- trainControl(method = train_method, number = 10, repeats = 3,allowParallel = TRUE)
		...
		training[,1] <- as.factor(training[,1])
		testing <- input[-intrain,]
		trctrl <- trainControl(method = train_method, number = 20, repeats = 10,allowParallel = TRUE)		
		tunegrid <- expand.grid(.mtry=c(1:maxmtry))
...
## RESULTS / SWITCH 
ttBTkNNalldata_ANN_ma2 = ttBTkNNalldata_ANN_ma
ttBTkNNalldata_ANN_ma2[ttBTkNNalldata_ANN_ma2 == "GROWTH"] = NA
ttBTkNNalldata_ANN_ma2[ttBTkNNalldata_ANN_ma2 == "MATUREDECLINER"] = NA

model1 = read.csv('ENSEMBLE_1_THRES36_FULLOUTPUT_GROWTHONLY.csv')
model2 = ttBTkNNalldata_ANN_ma2
write.csv(model2,paste("MOD2_RAW_OUTPUT","benchmark",82,1,".csv"))
model3 = SKmodcombine(model1=model1,model2=model2)
write.csv(model3,paste("MOD3_RAW_OUTPUT","benchmark",82,1,".csv"))

resultsmod2 = SKoutputs_mod2(model2)
SKoutputsave2(resultsmod2,paste("MOD2_OUTPUT","benchmark",82,1))
resultsmod3 = SKoutputs_mod2(model3,finalrun = TRUE)
SKoutputsave2(resultsmod3,paste("MOD3_OUTPUT","benchmark",82,1),finalrun = TRUE)

```

## ------------------------------------------------------ FUNCTIONS FOR FIRST MODEL (GROWTH / NOT GROWTH) ------------------------------------

# Function for converting model to output

```{r}

SKoutputs<- function(SKttBTkNNalldata_ANN = NULL){
#SWITCH
ttBTkNNalldata_ANN <- SKttBTkNNalldata_ANN

ttincprediction_ANN <- ifelse(ttBTkNNalldata_ANN == "GROWTH",1,0)
ttincprediction_ANN  <- FZmovingavg(ttincprediction_ANN ,maper = 12)
ttBTkNNalldata_ANN_ma <- ifelse(ttincprediction_ANN  != 0, "GROWTH",ttBTkNNalldata_ANN)

# Bench mark Accuracy/Precision test 
m = melt(ttBTkNNalldata_ANN_ma)
m$value = ifelse(m$value == "MATURECYCLICAL", "NOT_GROWTH", ifelse(m$value == "GROWTH", "GROWTH", "NOT_GROWTH"))
...
df_full = cbind(df_full_target,df_full_variables)
df_full = df_full[,3:ncol(df_full)]
df_full$value = ifelse(df_full$value == "MATURECYCLICAL", "NOT_GROWTH", ifelse(df_full$value == "GROWTH", "GROWTH", "NOT_GROWTH"))
df_full$value = as.factor(df_full$value)
df_full$TIME = df_full$`rep(colnames(BSASSETS_ANN), each = nrow(BSASSETS_ANN))`

final = cbind(data.frame(m$value),data.frame(df_full$value),data.frame(df_full$TIME))
final = final[complete.cases(final$m.value),]
final = final[complete.cases(final$df_full.value),]

conf_mat_bench = confusionMatrix(final$m.value,final$df_full.value)
conf_mat_bench
conf_mat_bench$byClass

# Bench mark per year distribution test

final2 = final %>% dplyr::select(m.value,df_full.TIME)
names(final2) = c("TARGET","TIME")
final2$TIME <- gsub('X', '', final2$TIME)
final2$TIME =  as.Date(final2$TIME, "%Y%m%d")
class(final2$TIME)

distanalysis <- function(df = NULL){
  dfdist = df %>% dplyr::select(TIME,TARGET)
  dfdist$TIME = format(dfdist$TIME,"%Y")
...
final2dist = distanalysis(final2)

#ggplot(data=final2dist, aes(x=TIME,y=perc)) + geom_point()

out = list()
out$cfm <- conf_mat_bench
out$dist <- final2dist
...
# Function to convert output to results and save file

```{r}

SKoutputsave <- function(output=NULL,filename){
t1 = data.frame(output$cfm$overall)
t2 = data.frame(output$cfm$byClass)
names(t2) = names(t1)
results = rbind(t1,t2)

write.csv(output$dist,paste(filename,'-dist.csv',sep=''))
write.csv(results,paste(filename,'-results.csv',sep=''))
...
# Function for model training - MODEL 1
...
SKhibacktestnonode_m1 <- function(trainingmx=NULL,predictionmx=NULL, class_method1=NULL, class_method2=NULL, train_method=NULL, useweights=FALSE,binary=FALSE,maxmtry = 35, node=NULL,twomodels=TRUE,seed = 82,para_sampl1 = NULL, para_sampl2 =NULL,outputprobs=FALSE){
	if(is.null(trainingmx) | is.null(predictionmx)){stop("FZhibacktest: Input is NULL")}
  require(e1071)
	require(caret)
  require(kernlab)
  ...
  set.seed(seed)
	...
	if(binary){
		...
		weightsvec <- weightsvec/sum(weightsvec)
		trctrl <- trainControl(method = train_method, number = 10, repeats = 3,allowParallel = TRUE)
		if(!is.null(para_sampl1)){ trctrl$sample = para_sampl1}
		tunegrid <- expand.grid(.mtry=c(1:maxmtry))
		...
		for(upos in uniquenames){
			...
			if(nrow(predict_indep_mat) != 0){
				colnames(predict_indep_mat)[-c(1,2)] <- colnames(testing[,-1]) 
				if(outputprobs == FALSE){
				  Prediction <- predict(fit, newdata = predict_indep_mat[,-c(1,2)])
				}
				else{
				  Prediction <- predict(fit, newdata = predict_indep_mat[,-c(1,2)],type="prob")
				  Prediction = unlist(Prediction[1])
				  }
				names(Prediction) <- predict_indep_mat[,1]
				subpos <- rownames(outmx)
				stockselect <- as.character(predict_indep_mat[,1])[as.character(predict_indep_mat[,1]) %in% subpos]
		...
		testing <- input[-intrain,]
		trctrl <- trainControl(method = train_method, number = 20, repeats = 10,allowParallel = TRUE)
		if(!is.null(para_sampl2)){ trctrl$sample = para_sampl2}
		tunegrid <- expand.grid(.mtry=c(1:maxmtry))
...
## ------------------------------------------------------- RUN FIRST MODEL (GROWTH / NOT GROWTH) --------------------------------------------

# MODEL - 1 - Create input table (N can be set to < 100% of data for fast processing test runs)

```{r}

N = 1.0

inputtraingrowthEVAlogit_s2 = sample_n(inputtraingrowthEVAlogit, nrow(inputtraingrowthEVAlogit)*N)
nrow(inputtraingrowthEVAlogit_s2)
...
## MODEL - 1 - ENSEMBLE1 APPROACH (KNN+GBA+XGBOOST - AVERAGE)
...
SKttBTkNNalldata_ANN_m1 <- SKhibacktestnonode_m1(trainingmx = inputtraingrowthEVAlogit_s2,predictionmx = predvalsgrowthEVAlogit_ANN,class_method1 = mod1,class_method2 = mod2,train_method = "repeatedcv",useweights = FALSE,binary=TRUE,twomodels=both,seed = mseed,para_sampl1 = para_sam1,para_sampl2 = para_sam2,outputprobs = TRUE)

knnprobs = SKttBTkNNalldata_ANN_m1
...
gbmprobs = SKttBTkNNalldata_ANN_m1
...
xgbprobs = SKttBTkNNalldata_ANN_m1

# ENSEMBLE 1 - FINAL MODEL

# TRY WITH 0.36

  knnprobs_matrix = matrix(as.numeric(knnprobs),nrow=nrow(knnprobs),ncol=ncol(knnprobs))
  gbmprobs_matrix = matrix(as.numeric(gbmprobs),nrow=nrow(gbmprobs),ncol=ncol(gbmprobs))
  xgbprobs_matrix = matrix(as.numeric(xgbprobs),nrow=nrow(xgbprobs),ncol=ncol(xgbprobs))
  
  ensemble_avg = (knnprobs_matrix + gbmprobs_matrix + xgbprobs_matrix) / 3
  rownames(ensemble_avg) = rownames(knnprobs)
  colnames(ensemble_avg) = colnames(knnprobs)
  
  threshold = 0.35
  ensemble_avg[ensemble_avg>threshold]="GROWTH"
  ensemble_avg[ensemble_avg<threshold]="MATURE"
  ensemble_avg
  write.csv(ensemble_avg,'ENSEMBLE_1_THRES36_FULLOUTPUT_GROWTHONLY.csv')

  output = SKoutputs(ensemble_avg)
  output
  SKoutputsave(output,'OUTPUT_ensemble-036threshold-100p-knn-gbm-xgb3.csv')


```

## MODEL - 1 - ENSEMBLE1 THRESHOLD MODELLING

```{r}

## SIMPLE ENSEMBLE -- AVERAGE SCORE BAGGING, WITH THRESHOLD CURVE 
...
for (t in thresholds){ 
  ...
  iteroutput = SKoutputs(ensemble_avg)
  
  Accuracy = iteroutput$cfm$overall[1]
  Acclist = append(Acclist,Accuracy)
  Precision = iteroutput$cfm$byClass[5]
  Prelist = append(Prelist,Precision)
  f1 = iteroutput$cfm$byClass[7]
  F1list = append(F1list,f1)
  meandist = mean(iteroutput$dist$perc)
  distlist = append(distlist,meandist)
  meanlastyear = mean(iteroutput$dist$perc[13:15])
  fdistlist = append(fdistlist,meanlastyear)
}

resultcurve = do.call(rbind, Map(data.frame, thresholds=thresholds,Acc=Acclist, Pre=Prelist,F1=F1list,yearlydist=distlist,yearlydist1517=fdistlist))
resultcurvemelt = melt(resultcurve,id.vars = 'thresholds', variable.name = 'metric')

ggplot(resultcurvemelt, aes(thresholds,value)) + geom_line(aes(colour = variable)) + scale_y_continuous(breaks=seq(0, 1, 0.05)) + ggtitle("ENSEMBLE_1")



## COMPARE WITH KNN ONLY
Acclist = c()
Prelist = c()
F1list = c()
distlist = c()
...
resultcurveknn = do.call(rbind, Map(data.frame, thresholds=thresholds,Acc=Acclist, Pre=Prelist,F1=F1list,yearlydist=distlist,yearlydist1517=fdistlist))
resultcurvemeltknn = melt(resultcurve,id.vars = 'thresholds', variable.name = 'metric')

resultscurveboth = cbind(resultcurve,resultcurveknn)
resultscurveboth = resultscurveboth[,-7]
resultcurvemeltboth = melt(resultscurveboth,id.vars = 'thresholds', variable.name = 'metric')

ggplot(resultcurvemeltboth, aes(thresholds,value)) + geom_line(aes(colour = variable)) + scale_y_continuous(breaks=seq(0, 1, 0.05)) +  ggtitle("KNN/ENSEMBLE") +  geom_hline(yintercept = 0.09)
dev.print(pdf, 'ensemblegraph.pdf')

```

## ------------------------------------------------------- FUCNTIONS FOR MODEL 2 (MATURE CYCLICAL / MATURE STABLE) ---------------------------

# Function to combine model 1 and 2 

```{r}
# COMBINE FIRST AND SECOND MODELS 

SKmodcombine <- function(model1 = NULL, model2 = NULL){
  stocknames = model1[1]
  model1 = model1[,2:ncol(model1)]
  combine <- ifelse(model1=="GROWTH","GROWTH",model2)
  rownames(stocknames) = stocknames$X
  rownames(combine) = rownames(stocknames)
  return(combine)
}

```

# Function for converting model 2 and 3 (combo of mod1 and 2) to output & switch rate analysis 

```{r}

SKoutputs_mod2<- function(mod2 = NULL,finalrun=FALSE){

ttBTkNNalldata_ANN <- mod2

ttBTkNNalldata_ANN_ma <- ttBTkNNalldata_ANN
...
if (finalrun == FALSE){
  df_full$value[df_full$value == "GROWTH"] = NA
}
df_full$value[df_full$value == "MATUREDECLINER"] = NA
df_full$value = as.factor(droplevels(df_full$value))
df_full$TIME = df_full$`rep(colnames(BSASSETS_ANN), each = nrow(BSASSETS_ANN))`
df_full$STOCK_ID = df_full$`rep(rownames(BSASSETS_ANN), ncol(BSASSETS_ANN))`
df_full$UNIQUE_ID = paste(df_full$STOCK_ID,df_full$TIME)

final = cbind(data.frame(m$value),data.frame(df_full$value),data.frame(df_full$TIME),df_full$STOCK_ID,df_full$UNIQUE_ID)
final = final[complete.cases(final$m.value),]
final = final[complete.cases(final$df_full.value),]
...
final2 = final %>% dplyr::select(-df_full.value)
names(final2)
names(final2) = c("TARGET","TIME","STOCK_ID","UNIQUE_ID")
final2$TIME <- gsub('X', '', final2$TIME)
final2$TIME =  as.Date(final2$TIME, "%Y%m%d")
...
df_switch = df_lag %>% select(TIME_Y, LAGBINARY) %>% group_by(TIME_Y) %>% summarise(switch_sum = sum(LAGBINARY,na.rm=T), count = n()) %>% mutate(perc_switch = switch_sum/count)
df_switch

out = list()
out$cfm <- conf_mat_bench
out$switch <- df_switch
return(out)
...
# Function to smooth switching 

```{r}

SKsmoothswitch <- function(output = NULL,filename){

ns = SKoutputs_mod2(output)

################ SWITCH METRICS 

# SMOOTHS GROWTH 
ttBTkNNalldata_ANN = SKttBTkNNalldata_ANN_m2
ttincprediction_ANN <- ifelse(ttBTkNNalldata_ANN == "MATURECYCLICAL",1,0)
ttincprediction_ANN  <- FZmovingavg(ttincprediction_ANN ,maper = 12)
ttBTkNNalldata_ANN_ma <- ifelse(ttincprediction_ANN  != 0, "MATURECYCLICAL",ttBTkNNalldata_ANN)

s1 = SKoutputs_mod2(ttBTkNNalldata_ANN_ma)

# SMOOTHS MS/MC
ttincprediction_ANN = ttBTkNNalldata_ANN_ma
ttincprediction_ANN <- ifelse(ttBTkNNalldata_ANN == "MATURESTABLE",1,0)
ttincprediction_ANN  <- FZmovingavg(ttincprediction_ANN ,maper = 12)
ttBTkNNalldata_ANN_ma <- ifelse(ttincprediction_ANN  != 0, "MATURESTABLE",ttBTkNNalldata_ANN)

s2 = SKoutputs_mod2(ttBTkNNalldata_ANN_ma)

switchresults = cbind(ns$switch[1],df_switch[7:nrow(df_switch),4],ns$switch[4],s1$switch[4],s2$switch[4])
colnames(switchresults) <- c("TIME","REAL_DATA","NO_SMOOTH","MC_SMOOTH","BOTH_SMOOTH")
switchresults
switchresults_melt = melt(switchresults,id="TIME")
switchresults_melt$TIME  = as.numeric(switchresults_melt$TIME)

ggplot(data=switchresults_melt,aes(x=TIME,y=value)) + geom_line(aes(color=variable,group=variable),size=2) + scale_x_continuous(limits=c(2003, 2014), breaks=c(2003:2014))+ scale_y_continuous(limits=c(0, 0.025)) + theme_dark() + geom_hline(yintercept = mean(switchresults$REAL_DATA,na.rm=TRUE))
...
# Function to save mod 2 and mod 3 (combo of mod1 and 2)

```{r}

SKoutputsave2 <- function(output=NULL,filename,finalrun=FALSE){

if (finalrun == TRUE){
  t1 = data.frame(output$cfm$overall)
  t2 = data.frame(output$cfm$byClass)
  t2g = melt(t2[1,])[2]
  rownames(t2g) = paste("G", names(t2))
  t2mc =  melt(t2[2,])[2]
  rownames(t2mc) = paste("MC", names(t2))
  t2ms =  melt(t2[3,])[2]
  rownames(t2ms) = paste("MS", names(t2))
  names(t1) = names(t2g)
  results = rbind(t1,t2g,t2mc,t2ms)
}
else{
t1 = data.frame(output$cfm$overall)
t2 = data.frame(output$cfm$byClass)
...
write.csv(output$switch,paste(filename,'-switch.csv',sep=''))
write.csv(results,paste(filename,'-results.csv',sep=''))
...
# Function for model training - MODEL 2

```{r}

# MODEL 2 FUNCTION 
SKhibacktestnonode_m2 <- function(trainingmx=NULL,predictionmx=NULL, class_method1=NULL, class_method2=NULL, train_method=NULL, useweights=FALSE, node=NULL,seed = 82,para_sampl1 = NULL, outputprobs=FALSE,maxmtry=35){
  require(e1071)
	require(caret)
  require(kernlab)
  require(LiblineaR)
...
## -------------------------------------------------------- RUN SECOND MODEL (MATURE CYCLICAL / MATURE STABLE) -------------------------------

# MODEL - 2 - create input table (N can be set to < 100% of data for fast processing test runs)
...
# SUBSET TRAINING DATA FOR ONLY MS/MC
inputtraingrowthEVAlogit_s3 = inputtraingrowthEVAlogit_s2
names(inputtraingrowthEVAlogit_s3)[names(inputtraingrowthEVAlogit_s3) == 'as.vector.ttmanualclass_ANNREP...Scolfortrain.Ecolfortrain..'] = "TARGET"
inputtraingrowthEVAlogit_s3 = inputtraingrowthEVAlogit_s3 %>% filter(is.na(TARGET) | TARGET != "GROWTH")
inputtraingrowthEVAlogit_s3 = inputtraingrowthEVAlogit_s3 %>% filter(is.na(TARGET) | TARGET != "MATUREDECLINER")
inputtraingrowthEVAlogit_s3$TARGET = droplevels(inputtraingrowthEVAlogit_s3$TARGET)
summary(inputtraingrowthEVAlogit_s3$TARGET)

```

# MODEL - 2 - Spot checking algorithms (STILL A WORK IN PROGRESS: MORE MODELS/PARAMETER TUNING TO DO DONE HERE)
...
SKttBTkNNalldata_ANN_m2  = SKhibacktestnonode_m2(trainingmx = inputtraingrowthEVAlogit_s3,predictionmx = predvalsgrowthEVAlogit_ANN,class_method1=mod1,train_method="repeatedcv",seed=mseed,para_sampl1 = NULL,outputprobs = FALSE)
...
resultsmod2 = SKoutputs_mod2(model2)
SKoutputsave2(resultsmod2,paste("MOD2_OUTPUT",mod1,mseed,n))
resultsmod3 = SKoutputs_mod2(model3,finalrun = TRUE)
SKoutputsave2(resultsmod3,paste("MOD3_OUTPUT",mod1,mseed,n),finalrun = TRUE)
...
# MODEL - 2 - Systematic caret algorithm loop (STILL A WORK IN PROGRESS: FOR LOOP TO TRY EVERY CLASSIFICATION MODEL - SOME MODELS VERY SLOW)

```{r}

Caret_ML_models_classification = Caret_ML_models %>% filter(Classification == 1)
Caret_ML_models_classification_test = Caret_ML_models_classification[c(1:3),]

for(i in 1:nrow(Caret_ML_models_classification_test)){
  # select row 
  row = Caret_ML_models_classification_test[i,]
  # install packages and load libraries
  mainlib = row$Library_1
  lapply(mainlib,require, character.only = TRUE)
  # select model 
  model = row$Method
  print(unlist(model))
  # apply model to data "
  ...
  mseed = 82
  # run model
  long_output = SKhibacktestnonode_m2(trainingmx = inputtraingrowthEVAlogit_s3,predictionmx = predvalsgrowthEVAlogit_ANN,class_method1=mod1,train_method="repeatedcv",seed=mseed,para_sampl1 = NULL,outputprobs = FALSE)
  # output results
  resultsmod2 = SKoutputs_mod2(long_output)
  print(resultsmod2$cfm)
}


```

# MODEL - 2 - Current best model 

```{r}

# MODEL1 = ENSEMBLE1_THRESHOLD36, MODEL2 = KNN (with smoothing), MODEL 3 = combo of mod1 and mod 2
mod1 = "knn"
para_sam1 = NULL
n = N
mseed = 82
...
SKttBTkNNalldata_ANN_m2 = ttBTkNNalldata_ANN_ma

# save output
model1 = read.csv('ENSEMBLE_1_THRES36_FULLOUTPUT_GROWTHONLY.csv')
write.csv(model1,paste("MOD1_RAW_OUTPUT_SMOOTH_FINAL","ENSEMBLE_1_THRES36",82,1,".csv"))
model2 = SKttBTkNNalldata_ANN_m2
write.csv(model2,paste("MOD2_RAW_OUTPUT_SMOOTH_FINAL",mod1,mseed,n,".csv"))
model3 = SKmodcombine(model1=model1,model2=model2)
write.csv(model3,paste("MOD3_RAW_OUTPUT_SMOOTH_FINAL",mod1,mseed,n,".csv"))

# save results 
resultsmod2 = SKoutputs_mod2(model2)
SKoutputsave2(resultsmod2,paste("MOD2_OUTPUT_SMOOTH_FINAL",mod1,mseed,n))
resultsmod3 = SKoutputs_mod2(model3,finalrun = TRUE)
SKoutputsave2(resultsmod3,paste("MOD3_OUTPUT_SMOOTH_FINAL",mod1,mseed,n),finalrun = TRUE)
